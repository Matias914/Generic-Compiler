...
TEST_F(TestParserLexer, programs)
{
    ...
    // 33, 37, 42, 46, 50, 51, 55, 55
    EXPECT_TRUE(ERROR_HANDLER.contains({ERROR, GLOBAL_SCOPE_STATEMENT  , 33, {""}}));
    EXPECT_TRUE(ERROR_HANDLER.contains({ERROR, MISSING_PROGRAM_NAME    , 37, {""}}));
    EXPECT_TRUE(ERROR_HANDLER.contains({ERROR, MISSING_PROGRAM_NAME    , 37, {""}}));
    ...
    EXPECT_TRUE(ERROR_HANDLER.contains({ERROR, MISSING_PROGRAM_NAME    , 55, {""}}));
    EXPECT_TRUE(ERROR_HANDLER.contains({ERROR, MISSING_CLOSING_BRACKET , 55, {""}}));

    EXPECT_EQ(ERROR_HANDLER.errorCount(), 8);
}